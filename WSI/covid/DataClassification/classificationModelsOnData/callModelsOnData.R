...
learnModel <- function(modelFun,fileNames,dirNamesSet){
  for (z in 1:length(modelFun)) {
    ...
    dirN = paste0("../../modelResults/",modelFun[z],"/")
    dir.create(dirN)
    ...
    for(i in 1:length(fileNames)) {
      ...
      for (j in 1:length(trainData)) {
        ...
        save(constantSet, output, Ytest,  file = paste0(path, j, ".RData"))
      }
    }
  }
}
...
modelFun <- c("logisticRegression","naiveBayesClassifier")


baseFolder = "../../generatedData/"
fileNames = list.files(baseFolder, full.names = TRUE)

dirSetting = "../../simulationSetting/"
